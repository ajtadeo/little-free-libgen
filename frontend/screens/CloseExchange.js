...
const CloseExchange = ({ navigation }) => {
  ...
  const removeBookFromDB = async () => {
    try {
      const response = await axios.post('http://' + uri + '/checkout', {
        isbn: isbn,
      })
      ...
      if (response.data) {
        ...
        Alert.alert("book couldn't be removed");
      }
      ...
      {
        {
          ...
          2:
            <View style={styles.container}>
              ...
              <View style={{ alignItems: 'center' }}>
                <View style={styles.button}>
                  <Button title="Lock Library" onPress={handleLockLibrary} color="white" />
                </View>
              </View>
              <TouchableOpacity
